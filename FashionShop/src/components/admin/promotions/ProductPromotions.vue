...
const fetchVariantDetails = async () => {
  const promises = promotions.value.map(async (item) => {
    try {
      const res = await axios.get(`http://localhost:8080/api/admin/product-variants/${item.productVariantId}`, {
        headers: { Authorization: `Bearer ${token}` }
      })
...
const saveUpdatedQuantity = async () => {
  ...
  try {
    await axios.put(`http://localhost:8080/api/admin/product-promotions/${selectedPromotion.value.id}`, selectedPromotion.value, {
      headers: { Authorization: `Bearer ${token}` }
    })
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="text-muted small">
          ...
          <strong>{{ variantMap[selectedPromotion.productVariantId]?.stock ?? '...' }}</strong>
        </div>
      </div>

      <div class="modal-footer">
        <button class="btn btn-secondary" @click="closeModal">Đóng</button>
        <button class="btn btn-primary" @click="saveUpdatedQuantity">💾 Lưu thay đổi</button>
      </div>
    </div>
  </div>
</div>
