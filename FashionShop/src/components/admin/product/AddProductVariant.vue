...
const fetchVariantDetail = async () => {
  ...
  isEdit.value = true
  try {
    const response = await axios.get(`http://localhost:8080/api/admin/product-variants/${variantId}`, {
      headers: { Authorization: `Bearer ${token}` }
    })
...
const handleSubmit = async () => {
  try {
    ...
    if (isEdit.value) {
      await axios.put(`http://localhost:8080/api/admin/product-variants/update/${variantId}`, formData, {
        headers: {
  ...
  <div class="container mt-4">
    ...
    <form @submit.prevent="handleSubmit" class="row g-3 mt-3">
      ...
      <div class="col-md-12">
        ...
        <input type="file" class="form-control" @change="e => variant.image = e.target.files[0]"  />
      ...
      <div class="col-12 d-flex justify-content-end">
        ...
        <button class="btn btn-success" type="submit">{{ isEdit ? 'Cập nhật' : 'Lưu' }}</button>
      </div>
    </form>
  </div>
</template>
